export default function MultiGradient (target, colors, blend) {
    ...
    if(validColors.length > 0 && ((colors.length == 4) || (colors.length == 6)) && validColors.length === colors.length) {
        ...
        let quad = validColors.length === 4
        let childWidth =   quad ? (fullWidth / 2) : (fullWidth / 3)
        createColorNode(parent, null, null, false, true, validColors)
        validColors.forEach((color, i) => {
            let position = getPosition(i, childWidth, quad) 
    ...
    function createColorNode (parent, color, position, width, isBackground, colorsArray, i) {
        ...
        if(isBackground) {
            let colorString = colorsArray[0] + ', ' + colorsArray[colorsArray.length - 1]
            let gradientString = `linear-gradient(to bottom right, ${colorString})`
