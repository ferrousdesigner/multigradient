...
exports.default = MultiGradient;

function MultiGradient(target, colors) {
  if (!target && !colors) {
  ...
  var validColors = colors.filter(function (color) {
    if (!color.match(/(#([0-9a-f]{3}){1,2}|(rgba|hsla)\(\d{1,3}%?(,\s?\d{1,3}%?){2},\s?(1|0?\.\d+)\)|(rgb|hsl)\(\d{1,3}%?(,\s?\d{1,3}%?\)){2})/i)) {
      console.log("%c Provided color '".concat(color, "' is not a valid color."), 'color: #ff6060');
    } else {
      return color;
    }
  });
  ...
  function createColorNode(parent, color, position, width, isBackground, colorsArray, i) {
    ...
    if (isBackground) {
      ...
      var gradientString = "linear-gradient(to bottom right, ".concat(colorString, ")");
